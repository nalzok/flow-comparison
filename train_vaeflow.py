...
def gaussian_nice_loglkhd(h, device):
    return -0.5 * torch.sum(torch.pow(h, 2), dim=1) - h.size(1) * 0.5 * torch.log(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train VAE with flow prior")
    parser.add_argument(
        "--dataset",
        default="mnist",
        dest="dataset",
        choices=("mnist", "cifar", "celeba", "Fashion-mnist"),
    ...
    parser.add_argument(
        "--device", dest="device", default="cpu", type=str, help="Device"
    )
    parser.add_argument(
        "--savedir",
        dest="savedir",
        default="./saved_models/mnist/vae_flow/beta_400",
    ...
    parser.add_argument(
        "--image_size", default=28, type=int, help="size of training image"
    )
    parser.add_argument(
        "--beta", dest="beta", default=400, help="weight of reconstruction loss"
    )

    # Flow settings:
    parser.add_argument(
        "--fc_dim", default=64, type=int, help="dimension of FC layer in the flow"
    )
    parser.add_argument(
        "--num_block",
        default=4,
    ...
    if args.loss_type == "cross_entropy":
        ...
        ), "Cross entropy should only be used for mnist or Fashion-mnist."
    ...
    optimizer1 = optim.Adam(modVAE.parameters(), lr=args.lr)
    trainable_parameters = [p for p in modFlow.parameters() if p.requires_grad]
    optimizer2 = torch.optim.Adam(
        trainable_parameters, lr=args.lr, betas=(0.8, 0.9), eps=1e-6, weight_decay=2e-5
    )
    for param in trainable_parameters:
        param.data = 0.05 * torch.randn_like(param)

    scheduler1 = torch.optim.lr_scheduler.StepLR(
        optimizer1, args.decay, gamma=0.5, last_epoch=-1
    )
    scheduler2 = torch.optim.lr_scheduler.StepLR(
        optimizer2, args.decay, gamma=0.5, last_epoch=-1
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(total=len(training_loader.dataset)) as progress_bar:
            for batch_idx, (dat, target) in enumerate(training_loader):
                ...
                if args.loss_type == "Perceptual" and (
                    args.dataset == "mnist" or args.dataset == "Fashion-mnist"
                ):
                ...
                progress_bar.set_postfix(
                    loss=np.mean(recon_losses),
                    logd=np.mean(log_D),
                    likloss=np.mean(like_losses),
                    entropy_loss=np.mean(entr_L),
                )
        ...
        print(
            "Train Epoch: {} Reconstruction-Loss: {:.4f} loglikelihood loss: {}  entropy loss: {}".format(
        ...
        if epoch % 50 == 0:
            torch.save(
                modVAE.state_dict(),
                os.path.join(args.savedir, "VAEModel_epo{}".format(epoch)),
            )
            torch.save(
                modFlow.state_dict(),
                os.path.join(args.savedir, "flowModel_epo{}".format(epoch)),
            )
