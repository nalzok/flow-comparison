...
def load_check_point(args,subnet_fc,device):
    ...
    state_flow = torch.load(os.path.join(args.model_dir, 'flowModel_epo100'), map_location = device)
    state_ae = torch.load(os.path.join(args.model_dir, 'AEModel_epo100'), map_location = device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", dest='model_dir', default="./saved_models/mnist/glf/",
                        help="Where to save the trained model.")
    parser.add_argument('--savedir', default='./samples/mnist', type=str, help='save image address')
    parser.add_argument("--num_latent",  default=20, type=int,
                        help="dimension of latent code z")
    
    parser.add_argument("--fc_dim", dest='fc_dim', default=64, type=int,
    ...
    for i in range(20):
        ...
        for j in range(500):
            sample[j,:,:,:] = (sample[j,:,:,:] - torch.min(sample[j,:,:,:])) / (torch.max(sample[j,:,:,:] - torch.min(sample[j,:,:,:])))
        sample = sample*255
        for j in range(sample.size(0)):
               t_utils.save_image(sample.view(500,3,h,w)[j, :, :, :],
                                            (args.savedir+'/sample/{}.png').format(j+i*500),normalize=True)
        print(i)
